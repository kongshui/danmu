...
// write写日志
func (logS *LogStruct) Write(label string, data string, debug bool) {
...
func (logS *LogStruct) sizeLogRorate(label string) {
	...
	if err != nil {
		logS.Write("error", "获取日志大小失败: "+err.Error(), false)
		return
	}
	if size <= 0 {
		return
	...
	if size >= int64(logS.maxSize) {
		if err := logS.logRotate(label); err != nil {
			logS.Write("error", label+"日志轮转失败: "+err.Error(), false)
		} else {
			logS.Write(label, label+"日志文件已轮转", false)
		}
	}
}
