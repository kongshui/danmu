...
- **liveGift,liveLike,LiveComment**
   ...
   **礼物格式如下所示：**
   ...
   27.4. 进入直播数据结构体
        {
            msg_id: "12345678312", // string类型的消息id
            enter_room_type: 1,      // 1-进入直播间；2-离开直播间
            sec_openid: "xxxx",     // 进房或离房用户openid，
            avatar_url: "xxx",     // 用户头像
            nickname: "xxxx",     // 用户昵称(不加密)
            is_old_player: 1,     // 是否是弹幕玩法老用户，0-新用户，1-老用户
            grade_level: "L2",    // 用户的直播贡献阶梯: L1-L6
            timestamp: 1649068964, // 进房或离房毫秒级时间戳（进房跟离房的消息不保证先后顺序，例如：离房在后，也有可能先收到推送消息。开发者需要自己根据时间戳做逻辑处理）
            inviter_gather_openid: "xxx",     // 通过用户一键摇人召集进入直播间，发起召集的用户openid（通过召集分享才有该字段）
            inviter_gather_nickname: "xxx",    // 发起召集的用户昵称
            inviter_gather_avatar_url: "xxx"   // 发起召集的头像
         },
...
- **PlayerSkillLevelUp 添加玩家技能等级协议**
   *protobuf结构体为：*
   message UpSkillLevelInfoMessage{
      string anchor_open_id = 1; // 主播OpenId
      repeated UpSkillLevelInfo up_skill_level_infos = 2; // 多个玩家单个技能升级经验
   }
   message UpSkillLevelInfo{
      string open_id = 1; // 玩家OpenId
      int32 skill_id = 2; // 技能Id
      float skill_experience = 3; // 技能升级经验
   }
   *文件名：* skill.proto
   **备注：** 此协议为添加玩家技能等级协议，其中anchor_open_id为主播OpenId，single_skill_experience_adds为多个玩家单个技能经验增加
- **PlayerSkillLevelUpAck 添加玩家技能等级回复协议**
   *protobuf结构体为：*
...
- **AddItemToUser 添加物品到用户协议**
   *protobuf结构体为：*
   // 添加物品信息消息
   message AddGoodsItemInfoMessage{
      string anchor_open_id = 1; // 主播OpenId
      repeated GoodsItemInfo add_item_infos = 2; // 多个物品增加信息
      string verify_key = 3; // 验证密钥
      int32 change_type = 4; // 物品类型变化类型 1<<0:增加 1<<1:减少,1<<2 重置等
   }

   message GoodsItemInfo{
      string open_id = 1; // 玩家OpenId
      int32 item_id = 2; // 物品Id
      int32 item_num = 3; // 物品数量
      int32 item_type = 4; // 物品类型
      string verify_key = 5; // 验证密钥
   }
   *文件名：* item.proto
   **备注：** 此协议为添加物品到用户协议，其中open_id为玩家用户id，items为物品信息列表
- **AddItemToUserAck 添加物品到用户回复协议**
   *protobuf结构体为：*
   message AddGoodsItemInfoAckMessage{
      string anchor_open_id = 1; // 主播OpenId
      string verify_key = 2; // 验证密钥
      int32 code = 3; // 状态码
      string msg = 4; // 状态信息
   }
   *文件名：* item.proto
   **备注：** 此协议为添加物品到用户回复协议，其中open_id为玩家用户id，items为物品信息列表
- **备注**
- **备注**
